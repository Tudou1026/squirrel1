        ...
        <div id="user_content">
            <div class="share">
                ...
                <div class="share_content">
                    <c:if test="${empty recordsAndImage}">
                        <div class="no_share">
                            <span>没有任何内容，去逛逛其它的吧！</span>
                        </div>
                    </c:if>
                    <c:if test="${!empty recordsAndImage}">
                        <c:forEach var="items" items="${recordsAndImage}">
                            <div class="story">
                                <a href="<%=basePath%>goods/goodsId/${items.record.goods.id}" class="head_img">
                                    <img src="../upload/${items.images[0].imgUrl}">
                                </a>
                                <span class="name">${items.record.goods.name}</span>
                                <span class="text" style="overflow: hidden; outline: none;">${items.record.goods.describle}</span>
                                <div class="box">
                                    ...
                                    <div class="interact">
                                        ...
                                        <%--<span class="time">${items.goods.createdAt}</span>--%>
                                        <span class="fa fa-trash"><a href="<%=basePath%>goods/deleteRecords/${items.record.id}">删除</a></span>
                                    </div>
                                    <div class="like_detail">
                                        <c:if test="${items.record.commentStatus==false}">
                                        <div class="like_content">
                                            <span>购物时间：${items.record.createdAt}</span>
                                        </div>
                                        </c:if>
                                        <c:if test="${items.record.commentStatus!=false}">
                                            <div class="like_content">
                                            ...
                                            <div class="like_content">
                                                <span>评价时间：${items.record.updatedAt}</span>
                                            </div>
                                        </c:if>
                                    </div>
                                </div>
